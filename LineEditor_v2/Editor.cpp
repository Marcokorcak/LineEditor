...
Editor::Editor(string _fileName, const string _fileName2) //Constructor to initialize aray, and two files
{
	...
	while (!in.eof())
	{
		in >> keyWords[numKeywords]; //Take in each keyword in the text file and place them into the indeces of the array
		numKeywords++; //Increment the number of keywords by one each time
	}
	bubbleSort(keyWords, numKeywords); //Use bubbleSort to sort the array of keywords
}
